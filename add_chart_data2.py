...
from video.models import PopVideo
...
def parse_and_append(chart_date):
    ...
    driver.get('https://www.melon.com/chart/age/list.htm' +
               '?idx=2' + '&chartType=YE' + '&chartGenre=POP' + '&chartDate=' + str(chart_date) + '&moved=Y')
    ...
    for i in range(len(title)):
        videos = PopVideo.objects.filter(type=chart_date, title=title[i]).order_by('chart')
        if len(videos) == 0:
...
def parse_and_append2(video_type, chart, title, artist):
    ...
    PopVideo(type=video_type, chart=chart, title=title, artist=artist, video_key=key).save()
    time.sleep(2)
